...
```
Now that we have the properties defined, return to the desktop editor, navigate to the `Sounds` tab under `Build`, and change the dropdown to `Music`. Then you can choose two tracks to add into your world, for my example I will use `Arcade Theme` for the lobby and `Flexin` for the game area. Adjust the volume to your liking and disable the `Play on Start` option. Once you are happy, click on the `GameController` object and link your sound assets to the `lobbySound` and `playingSound` properties.
