...
In this five part tutorial series, we'll expand on the concepts from my previous guide, [Horizon World Tutorial - Player Management](../player-manager), to create a maze runner game. We'll walk through setting up a new world, building a game controller, adding sound, generating a maze, and adding NPC runners. Before starting, make sure you've completed the `Player Management` tutorial and are ready to duplicate that world in your Horizon Worlds desktop editor.
